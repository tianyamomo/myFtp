...
/***
 * 采用alibaba的Druid创建数据库连接池
 * 用来插入到数据库中
 *
 *这个Class 需要加载的jdbc.properties的属性如下（oracle）：
 *Ins.driverClassName ： 数据库驱动
 *Ins.url ： 数据库连接串
 *Ins.username ： 连接数据库的账号
 *Ins.password ： 连接数据库账号的密码
 ...
 *
 */
public class InsJdbcUtils {
	private static Logger logger = Logger.getLogger(InsJdbcUtils.class);
	private static DruidDataSource dataSource;
	private static InsJdbcUtils instance = null;
	private static Connection con = null;
	private DataSourceParameterEntity dspe = new DataSourceParameterEntity();
	
	static{
		instance = new InsJdbcUtils();
	}
	
	private InsJdbcUtils(){
